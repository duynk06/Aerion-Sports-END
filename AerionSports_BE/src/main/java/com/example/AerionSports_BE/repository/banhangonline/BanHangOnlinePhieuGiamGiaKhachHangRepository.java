...
public interface BanHangOnlinePhieuGiamGiaKhachHangRepository extends JpaRepository<PhieuGiamGiaKhachHang, Integer> {
    ...
    @Query("""
        SELECT CASE WHEN COUNT(pgk) > 0 THEN true ELSE false END FROM PhieuGiamGiaKhachHang pgk
        WHERE pgk.phieuGiamGia.id = :idPhieuGiamGia
    """)
    boolean existsPhieuGanKhachHang(@Param("idPhieuGiamGia") Integer idPhieuGiamGia);
    ...
    """)
    boolean existsKhachHangDuocDungPhieu(
