...
public class BanHangController {
    ...
    public ResponseEntity<List<BanHangResponse>> getHoaDonCho() {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String username = auth != null ? auth.getName() : null;
        return ResponseEntity.ok(banHangService.getHoaDonCho(username));
    }
    ...
    public ResponseEntity<?> thanhToan(@RequestBody ThanhToanRequest request) {
        try {
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
            String username = auth != null ? auth.getName() : null;
            return ResponseEntity.ok(banHangService.thanhToan(request, username));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> huyHoaDon(@PathVariable Integer id) {
        try {
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
            String username = auth != null ? auth.getName() : null;
            banHangService.huyHoaDon(id, username);
            return ResponseEntity.ok().build();
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    ...
    @ResponseBody
    public ResponseEntity<?> themKhachHangNhanh(@RequestBody KhachHangNhanhRequest request) {
        try {
            return ResponseEntity.ok(banHangService.themKhachHangNhanh(request));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    }
