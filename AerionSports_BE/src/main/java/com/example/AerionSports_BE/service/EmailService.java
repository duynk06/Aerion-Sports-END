...
public class EmailService {
    ...
    public void sendAccountCreationEmail(String toEmail, String tenNhanVien, String matKhauTamThoi) {
        try {
            ...
            String htmlContent = "<div style='font-family: Arial, sans-serif; padding: 25px; border: 1px solid #f79b66; border-radius: 12px; max-width: 550px; margin: 0 auto;'>"
                    ...
                    + "  <h2 style='color: #f79b66; margin: 0; font-size: 22px; letter-spacing: 1px;'>CHÀO MỪNG NHÂN VIÊN MỚI</h2>"
                    ...
                    + "  <p style='margin: 5px 0; font-size: 14px;'>🌐 <strong>Trang quản trị:</strong> <a href='http://localhost:8080/login' style='color: #ea712b; text-decoration: none; font-weight: bold;'>Click để đến trang Đăng nhập</a></p>"
    ...
    /**
     * Gửi email cấp tài khoản và mật khẩu cho Khách hàng mới đăng ký
     */
    @Async
    ...
    /**
     * 🆕 Gửi email mật khẩu mới khi người dùng thực hiện chức năng "Quên mật khẩu"
     * @param toEmail Email của tài khoản (dùng chung cho cả nhân viên và khách hàng)
     * @param tenNguoiDung Tên hiển thị của người dùng
     * @param matKhauMoi Mật khẩu mới do hệ thống tự sinh
     */
    @Async
    public void sendResetPasswordEmail(String toEmail, String tenNguoiDung, String matKhauMoi) {
        try {
            ...
            helper.setTo(toEmail);
            helper.setSubject("🔑 Aerion Sports - Yêu cầu cấp lại mật khẩu");

            String htmlContent = "<div style='font-family: Arial, sans-serif; padding: 25px; border: 1px solid #f79b66; border-radius: 12px; max-width: 550px; margin: 0 auto;'>"
                    + "<div style='text-align: center; margin-bottom: 20px;'>"
                    + "  <h2 style='color: #f79b66; margin: 0; font-size: 22px; letter-spacing: 1px;'>YÊU CẦU CẤP LẠI MẬT KHẨU</h2>"
                    + "  <p style='color: #475569; font-size: 14px;'>Hệ thống đã tạo mật khẩu mới cho tài khoản của bạn</p>"
                    + "</div>"
                    ...
                    + "<div style='background-color: #f8fafc; padding: 18px; border-radius: 8px; border: 1px solid #e2e8f0; margin: 15px 0; line-height: 1.6; text-align: center;'>"
                    + "  <span style='font-weight: 700; font-size: 18px; color: #dc2626; font-family: monospace; background: #fee2e2; padding: 6px 14px; border-radius: 6px;'>" + matKhauMoi + "</span>"
                    + "</div>"
                    + "<p style='color: #ef4444; font-size: 12.5px; font-style: italic; font-weight: 500;'>⚠️ Nếu bạn không thực hiện yêu cầu này, vui lòng liên hệ ngay với quản trị viên. Vì lý do bảo mật, bạn nên đổi mật khẩu ngay sau khi đăng nhập.</p>"
                    + "<hr style='border: none; border-top: 1px solid #f1f5f9; margin: 20px 0;'/>"
                    + "<p style='font-size: 11px; color: #94a3b8; text-align: center; margin: 0;'>Hệ thống vận hành tự động Aerion Sports &copy; 2026</p>"
                    + "</div>";
            ...
            helper.setText(htmlContent, true);
            mailSender.send(message);
            System.out.println(">>> [MAIL SUCCESS] Đã gửi mật khẩu mới tới: " + toEmail);
        } catch (Exception e) {
            System.err.println(">>> [MAIL ERROR] Thất bại khi gửi mật khẩu mới tới " + toEmail + ". Lý do: " + e.getMessage());
        }
    }
